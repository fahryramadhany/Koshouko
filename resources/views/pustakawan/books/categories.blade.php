...
@section('title', 'Kelola Kategori Buku - Perpustakaan Digital')
...
<link rel="stylesheet" href="{{ asset('css/admin-pages.css') }}">
    ...
    <div id="addForm" class="hidden mb-6 gradient-card rounded-2xl p-8">
        ...
        <form action="{{ route('librarian.books.categories.store') }}" method="POST" class="space-y-4">
            @csrf
                        ...
                        <tr class="hover:bg-koshouko-cream/50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('librarian.books.categories.edit', $category) }}" class="text-koshouko-wood hover:underline text-sm font-semibold">Edit</a>
                                    <form action="{{ route('librarian.books.categories.destroy', $category) }}" method="POST" style="display:inline;">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center">
                                <p class="text-koshouko-text-muted">Belum ada kategori. Klik tombol "Tambah Kategori" untuk membuat kategori baru.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
