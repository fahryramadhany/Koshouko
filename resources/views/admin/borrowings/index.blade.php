...
<link rel="stylesheet" href="{{ asset('css/borrowings-fix.css') }}">
    ...
    <!-- Search & Filter Bar -->
    <div class="mb-6 gradient-card rounded-2xl p-4">
        <form action="{{ route('admin.borrowings') }}" method="GET" class="flex flex-col sm:flex-row gap-3 items-center">
            <!-- Search -->
            ...
            @if(request('search') || request('status'))
                <a href="{{ route('admin.borrowings') }}" class="px-4 py-2 border-2 border-koshouko-border text-koshouko-text rounded-lg font-semibold transition text-center hover:bg-koshouko-cream">
                    Reset
                </a>
            @endif
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (reportForm) {
            reportForm.addEventListener('submit', function(e) {
                ...
                generatePDFReport('{{ route('admin.borrowings.report') }}', reportType, 'Laporan-Peminjaman');
            });
        }
    });
    </script>
